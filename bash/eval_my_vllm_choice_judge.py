...
class Mytest_Evaluator_vllm(Evaluator):
    ...
    def generate_batch(
        ...
        if cot:
            sampling_params = SamplingParams(temperature=temperature, top_p=top_p, top_k=1, max_tokens=max_gen_len, stop='\n\n')
        else:
    ...
    def generate_batch_judge(
        ...
        if cot:
            ...
            sampling_params = SamplingParams(temperature=0, top_k=-1, max_tokens=4)
        outputs = self.llm.generate(prompts, sampling_params)
        answers = [output.outputs[0].text for output in outputs]
            # import pdb;pdb.set_trace()
        print('此任务测试完成')
...
def load(
    ...
    llm = LLM(model=ckpt_dir, tokenizer=token_dir, tokenizer_mode="slow", dtype="float16", gpu_memory_utilization=0.9, trust_remote_code=True)
